...
function SWEP:DrawHUD()
	...
	local view = Immersivecam(ply) or {
		angles = Angle(0, 0, 0)
	}
...
function SWEP:Step()
	...
	self.animLerp = LerpAngle(0.25, self.animLerp, Angle(5, 0, self.HoldType == "revolver" and 0 or -2) * self.animProg / 0.87)
	local ply = self:GetOwner()
...
function SWEP:Reload()
    ...
    -- Создаем таймер для перезарядки
    timer.Create(

...
function Bleeding()
	hook.Add(
		...
		function(victim, dmginfo)
			victim.Blood = victim.Blood or 1600
	...
	hook.Add(
		...
		function()
			for i, ply in ipairs(player.GetAll()) do
				...
				local pulse = math.Clamp((2 - ply.Blood / 1600) - ((ply.Bloodlosing + ply.arterybloodlosing) / 250), 0.1, 1)
				if not (ply.BLEEDING_NextThink > CurTime()) then
					...
					if ply.HasLeft == nil then
						...
						ply:SetWalkSpeed((120 * (ply.Blood / 1600)) * (ply.stamina / 100) * ply.RightLeg * ply.LeftLeg)
						ply:SetRunSpeed((210 * (ply.Blood / 1600) + (ply.Speed * 50)) * (ply.stamina / 100) * ply.RightLeg * ply.LeftLeg)
						ply:SetJumpPower(((190 * (ply.Blood / 1600) + (ply.Speed * 25)) * (ply.stamina / 100)) * ply.RightLeg * ply.LeftLeg)
					end
				end
			end
		end

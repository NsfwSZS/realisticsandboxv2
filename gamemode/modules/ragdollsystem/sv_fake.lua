...
concommand.Add(
	...
	function(ply)
		...
		if ply.pain > (250 * (ply.Blood / 1600)) + (ply:GetNWInt("SharpenAMT") * 5) or ply.Blood < 320 then return end
		timer.Create("faketimer" .. ply:EntIndex(), 2, 1, function() end)
		if ply:Alive() then
			Faking(ply)
			ply.fakeragdoll = ply:GetNWEntity("DeathRagdoll")
		...
		for i, ply in ipairs(player.GetAll()) do
			...
			if not (ply.OrgansNextThink > CurTime()) then
				...
				if ply.Organs and ply:Alive() then
					...
					--print(ply.Blood)
					if ply.Blood and ply.Blood <= 640 and ply:Alive() or ply.pain and ply.pain > 950 and ply:Alive() then
						ply:ExitVehicle()
						ply:Kill()
						ply.Bloodlosing = 0

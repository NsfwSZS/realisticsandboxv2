	...
	hook.Add(
		...
		function(ply, dmginfo)
			if ply:IsPlayer() or IsValid(RagdollOwner(ply)) then
				...
				if (ply.pain > (250 * (ply.Blood / 1600)) + (ply:GetNWInt("SharpenAMT") * 5) or ply.Blood < 960) and not ply.Otrub then
					ply.gotuncon = true
				end
			end
		end
...
function IsUnconscious(ply)
	if ply.pain > (250 * (ply.Blood / 1600)) + (ply:GetNWInt("SharpenAMT") * 5) or ply.Blood < 960 then
		ply.Otrub = true
	else
		ply.Otrub = false
	end
